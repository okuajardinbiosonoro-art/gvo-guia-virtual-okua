# FINAL-ACCESS-IV-001 — Production brief
...
Memoria de Mesa de sistema.

## 4. Función visual

Mini escena flotante con mesa técnica y red de nodos resumida.
...
/final / acceso IV; imagen decorativa dentro del botón; z30.
...
Silueta dentro del 14–86 %; alpha exterior 10–16 %; red contenida dentro de la superficie.
...
Mesa/superficie pequeña; 3–5 nodos resumidos; una relación de red clara; base material común.

## 16. Contenido prohibido

Dashboard, texto, cables densos, captura W4, monitor dominante, red completa o pulsos activos horneados.
...
10. `W4C` — `Contact sheet Mundo IV` — `docs/visual/final/021b-preproduction/final_021b_world4_memory_candidates.png`
11. `W4T` — `W4 superficie de mesa` — `public/assets/gvo/current-used/world-4-root/table/world4_table_top_v01.png`
12. `W4R` — `W4 ruta de sistema` — `public/assets/gvo/current-used/world-4-root/route/world4_system_route_base_v01.png`
13. `W4N` — `W4 nodo central` — `public/assets/gvo/current-used/world-4-root/objects/world4_node_central_system_v01.png`
...
W4C: síntesis de familia; W4T: masa de mesa; W4R: arco de red; W4N: jerarquía de nodo. Common: materialidad.

## 20. Qué no copiar

No reutilizar mesa, ruta ni nodo; no replicar la mesa runtime completa. H07 permanece `OPEN_CONTROLLED_ART_DIRECTION_ONLY_NO_BINARY_REUSE`.

## 21. Prompt positivo en inglés

```text
Create one independent floating memory mini-scene for the Final Mirador: a small floating stone-and-wood technical table with three to five simplified connected nodes, immediately readable as a compact system. Clean warm poetic pixel art, shared Mirador stone and vegetation materiality, transparent square asset, strong readable silhouette, mobile readability at 88 px, same apparent pixel scale as the other four access assets, controlled detail, compatible with portrait and landscape, no copied full-world scene, no UI, no character, no background environment. The visible subject must occupy 68–78 percent of both canvas width and height, keep 10–16 percent transparent alpha margin on every side, and keep the optical center within 8 percent of canvas center. Use restrained contact shadow only inside the safe area.
```
...
Mesa y red funcionan como una masa 68–78 %; la lectura no depende de líneas de menos de 3 px en el source. Crop sólo de alpha exterior; crop de silueta o de una parte
...
Export exactly `final_access_world4_system_v01.webp` as 1024x1024 WebP with alpha; preserve apparent pixel scale; preliminary budget <=180 KiB.
...
Acceso III producido y revisado; Art Bible/cámara; Environment auditado.

## 33. Estado

`READY_FOR_HUMAN_ASSET_PRODUCTION`. Este estado declara completo el brief, no
el asset. Orden de producción: `4`. No saltar dependencias.
