# FINAL-ACCESS-V-001 — Production brief
...
Memoria de Mapa del presente.

## 4. Función visual

Mini escena flotante con mapa/tablero resumido y zona preparada para ping.
...
/final / acceso V; imagen decorativa dentro del botón; z30.
...
Silueta dentro del 14–86 %; alpha exterior 10–16 %; marcador futuro dentro del bbox y lejos del label.
...
Mapa/tablero compacto; síntesis espacial; marcador o zona preparada para ping; base material común.

## 16. Contenido prohibido

Texto, geografía nueva, mapa runtime completo, controles, cuatro áreas copiadas, ping animado horneado.
...
10. `W5C` — `Contact sheet Mundo V` — `docs/visual/final/021b-preproduction/final_021b_world5_memory_candidates.png`
11. `W5E` — `W5 base de mapa portrait` — `public/assets/gvo/current-used/world-5-root/world5_map_environment_portrait_v01.webp`
12. `W5P` — `W5 sector Plantas` — `public/assets/gvo/current-used/world-5-root/world5_map_sector_plants_v01.webp`
13. `W5S` — `W5 sector Sistema` — `public/assets/gvo/current-used/world-5-root/world5_map_sector_system_v01.webp`
...
W5C: síntesis visual; W5E: idea de mapa/cavidad; W5P/W5S: vocabulario de sectores. Common: cámara y materialidad.

## 20. Qué no copiar

No copiar el mapa, cavidad, plantas, dispositivo ni organización de cuatro áreas. H07 permanece `OPEN_CONTROLLED_ART_DIRECTION_ONLY_NO_BINARY_REUSE`.

## 21. Prompt positivo en inglés

```text
Create one independent floating memory mini-scene for the Final Mirador: a small floating stone-and-wood observation map board with a simple abstract spatial layout and one restrained clear area reserved for a future ping. Clean warm poetic pixel art, shared Mirador stone and vegetation materiality, transparent square asset, strong readable silhouette, mobile readability at 88 px, same apparent pixel scale as the other four access assets, controlled detail, compatible with portrait and landscape, no copied full-world scene, no UI, no character, no background environment. The visible subject must occupy 68–78 percent of both canvas width and height, keep 10–16 percent transparent alpha margin on every side, and keep the optical center within 8 percent of canvas center. Use restrained contact shadow only inside the safe area.
```
...
Tablero y base ocupan 68–78 %; borde y zona de ping se distinguen a 88 px sin texto. Crop sólo de alpha exterior; crop de silueta o de una parte
...
Export exactly `final_access_world5_map_v01.webp` as 1024x1024 WebP with alpha; preserve apparent pixel scale; preliminary budget <=180 KiB.
...
Acceso IV producido y revisado; Art Bible/cámara; Environment auditado.

## 33. Estado

`READY_FOR_HUMAN_ASSET_PRODUCTION`. Este estado declara completo el brief, no
el asset. Orden de producción: `5`. No saltar dependencias.
