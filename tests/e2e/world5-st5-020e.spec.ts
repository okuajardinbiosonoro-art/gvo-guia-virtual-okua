...
test("ST5-020E compone encabezado y cuatro clusters dentro del recinto", async ({
  ...
  await page.goto("/estacion/5", { waitUntil: "domcontentloaded" });
  await expect(root(page)).toHaveAttribute("data-station5-state", "map_overview");
  ...
  await expect(page.getByText("Mapa del presente", { exact: true })).toHaveCount(0);
    ...
    return [...document.querySelectorAll<HTMLElement>(".s5-sector")].map(
      (sector) => {
        ...
        return {
          ...
          normalizedCenter: {
            x: ((image.left + image.right) / 2 - artboard.left) / artboard.width,
            y: ((image.top + image.bottom) / 2 - artboard.top) / artboard.height,
          },
        };
      },
    );
  ...
  for (const sector of geometry) {
    expect(Math.min(sector.target.width, sector.target.height)).toBeGreaterThanOrEqual(44);
...
test("ST5-020E recupera stage y compacta tarjeta en móviles contractuales", async ({
  ...
  for (const viewport of [
    ...
    expect(measured.stageHeight / measured.availableHeight).toBeGreaterThanOrEqual(0.68);
...
test("ST5-020E asienta P_PLANT_BASE dentro de SOIL_DARK_BAND", async ({ page }) => {
  for (const height of [548, 667, 548]) {
    await page.setViewportSize({ width: 375, height });
    await page.goto("/estacion/5/plantas", { waitUntil: "domcontentloaded" });
    await expect(root(page)).toHaveAttribute("data-station5-state", "plants_intro");
    await expect(root(page)).toHaveAttribute("aria-labelledby", "station5-title");
...
test("ST5-020E conserva estado y proyección en 360x560 → 360x640 → 360x560", async ({
  ...
  await expect(root(page)).toHaveAttribute("data-station5-state", "system_intro");
  const samples = [];
...
test("ST5-020E conserva el contacto de Plantas en 360x560 → 360x640 → 360x560", async ({
  ...
  expect(
    samples.every((sample) => Math.abs(sample.normalizedSourceY - 0.6384) < 0.01),
  ).toBe(true);
});
