...
async function probe(name, browserType, launchOptions = {}) {
  try {
    ...
    await page.evaluate(() => localStorage.setItem("gvo.station5.v1", JSON.stringify({ schemaVersion: 1, completedAreas: ["plantas", "sistema", "espacio"], updatedAt: "2026-07-30T12:00:00.000Z" })));
    await page.goto("http://127.0.0.1:4173/estacion/5/visitante");
    ...
    const copyVisible = await page.getByText("Toca la presencia dentro del aro.").isVisible();
    const focusVisible = await page.getByRole("button", { name: "Reconocer la presencia del visitante dentro del recorrido." }).isVisible();
    await browser.close();
    return { name, available: true, version, state, dimensions, copyVisible, focusVisible, pass: state === "visitor_intro" && copyVisible && focusVisible && dimensions.client.join("x") === dimensions.scroll.join("x") };
  } catch (error) {
...
await fs.writeFile(path.resolve("docs/visual/world5/st5-020g/browser_matrix.json"), `${JSON.stringify(result, null, 2)}\n`);
