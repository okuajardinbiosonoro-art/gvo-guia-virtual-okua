...
result.pass = assets.length === 21 && assets.every((asset) => asset.precached) && shellEntries.every((entry) => entry.precached) && result.navigateFallback;
...
await fs.writeFile(path.resolve("docs/visual/world5/st5-020f/pwa_precache.json"), `${JSON.stringify(result, null, 2)}\n`);
